...
def categorical(df):
    ...
    print(categorical_var.columns.values)
...
def numerical(df):
    ...
    print(numerical_var.columns.values)
...
def clear(df,col,val):
    variable_dist  = df[col].value_counts()[val]
...
def agg_values_ina_month(df,date_col,agg_col,agg):
    ...
    pvt_table = pd.pivot_table(df ,index = 'Month'  , values = agg_col ,aggfunc = agg )
    df = df.drop(columns = 'Month' , axis = 1)
    '''
    pvt_table = pd.pivot_table(df ,index = date_col  , values = agg_col ,aggfunc = agg )
    return pvt_table
...
def group_values(df,col1,agg1):
    ...
    #print("grouping cols: ",grouping.columns.values)
    return grouping
...
def convert(df,celsius):
    converted_temp = df[celsius].apply(lambda x:(x * 1.8) + 32)
...
weather['Month'] = pd.to_datetime(weather['Date/Time']).dt.month

pvt_table = agg_values_ina_month(weather,'Month','Temp (C)',[np.mean , max, min, sum, len])

weather = weather.drop(columns = 'Month' , axis = 1)
'''
print(pvt_table)
'''
